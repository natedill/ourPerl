...
package ElementQuadTreeBingMaps;
...
sub new_from_adcGrid
{
      ...
      my $east=$args{-EAST};
      my $west=$args{-WEST};
 	  $obj->{QTreeID}[0] = $args{-TILEPREFIX};    # prefix for fist tile specified
      ...
      print "new_from_adcGrid: added elements from adcGrid\n";
...
sub _divideRegion {
	...
	my ($recurseDepth,$index) = @_;
        my ($north, $south, $east, $west) = @{$obj->{REGION}[$index]};
        my $QTid = $obj-> {QTreeID} [$index];

        print "dividing region - index: $index, depth: $recurseDepth, QTreeID: $QTid \n";
    	print "region: $north $south $east $west\n";

	$recurseDepth++;
	
	my $top = &geo2pY($north,$recurseDepth);
	my $bottom = &geo2pY($south,$recurseDepth);

	my $mid=($top+$bottom)/2.0;
	#print "y coords: $top $mid $bottom \n";
		
	my $ew =  ($east + $west)/2.0;
	#my $ns =  ($north2 + $south2)/2.0;
	my $ns = &pY2geo($mid,$recurseDepth);
...
#######################################################
# sub Geo2pXY()-  public method
#
# converts lat/lon coordinates to pixel  
#######################################################
sub geo2pY{
	my ($Lat,$depth)=@_;
	my $pi=3.14159265359;
	my $rad = $Lat*$pi/180;  
	my $y = log(abs(tan($rad)+sec($rad)));
	return($y);
}
...
# sub geo2pXY{
	# my ($Lon,$Lat,$depth)=@_;
	# my $pi=3.14159265359;
    # my $mapsize=256*2**$depth;
	# my $sinLatitude = sin($Lat * $pi / 180);  
    # my $x = (($Lon + 180) / 360) * $mapsize;
	# my $y = 0.5 - ( log( ( 1 + $sinLatitude) / (1 - $sinLatitude) ) / (4 * $pi) ) * $mapsize;
	# return($y);
# }
...
# sub pXY2geo{
	...
	# my $mapsize=256*2**$depth;
	# my $y2= 0.5-$y/($mapsize);
	# my $Lon = ($x/(256*2**$depth)) * 360 - 180;
	# my $Lat = 90-360*atan(exp(-$y2*$pi))/$pi;
	# return($Lat);
# }
#######################################################
...
#					 -MAXDEPTH             #optional value for maximum recursiveDepth
#                 ) 
#   input is a hash
#
...
sub _bottomUp {
    ...
    if ($allFour==4) {
       print "BOTTOM UP: making DEMBLOCK for $index\n";
       print "BOTTOM UP: my kids are @kids\n";
...
sub _topDown {
    ...
    my ($obj, $index, $depth,$maxdep)=@_;
    my @kids = @{$obj->{CHILDREN}[$index]};
    my $allFour=0;
	my $k=0;  #to iterate through kids
	
	#check if at max depth, and return
	if ($depth = $maxdep){
	return
	}
	#check if there are 4 children
    foreach my $kid (@kids) {
 	   $allFour++ if (defined $obj->{DEMBLOCK}[$kid] );
    }
	if ($allFour==4){
	...
	#if all 4 exist, go back up
	else {
		
	}
	
	if 
	print "TOP DOWN: DEMBLOCK $index, KID $k \n";
...
###########################################################
    ...
    if ($allFour==4) {
       print "TOP DOWN: making DEMBLOCK for $index\n";
       print "TOP DOWN: my kids are @kids\n";
...
#	print "cnt is $cnt\n";
	...
	# print FILE2 $im->png;
	# close(FILE2);
    my $pngFile2= "FILES/$self->{SETNAME}/im_$QTreeID.png";  #SJH - print index, etc. 
	open FILE3, ">$pngFile2";
	binmode FILE3;
	print FILE3 $im->png;
	close(FILE3);
